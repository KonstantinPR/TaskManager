...
//  * bookmarks and pages of content;
//  * text hyphenation;
 ...
 * <li>bookmarks and pages of content;</li>
...
class TCPDF {
	...
	/**
	 * @var Default number of columns for html pages.
	 * @access protected
	 */
	...
	/**
	 * Array of: X difference between pages cell x start and starting page margin, cellspacing, cellpadding
	 * @access protected
	 * @since 5.8.000 (2010-08-11)
	 */
	...
	/**
 	 * This method is used to render the pages header on new page (if any).
 	 * @access protected
	...
	protected function setTableHeader() {
		...
		if (!$this->empty_string($this->thead) AND (!$this->inthead)) {
			...
			// set new top margin to skip the pages headers
			if (!isset($this->theadMargins['top'])) {
	...
	protected function _getTrueTypeFontSubset($font, $subsetchars) {
		...
		// get the offsets to the locations of the glyphs in the font, relative to the beginning of the glyphData pages
		$indexToLoc = array();
		...
		// get glyphs indexes of chars from cmap pages
		...
		foreach ($encodingTables as $enctable) {
			...
			switch ($format) {
				case 0: { // Format 0: Byte encoding pages
				...
				case 2: { // Format 2: High-byte mapping through pages
				...
				case 6: { // Format 6: Trimmed pages mapping
					$offset += 4; // skip length and version/language
		...
		// build new glyf pages with only used glyphs
		$glyf = '';
		$glyfSize = 0;
		// create new empty indexToLoc pages
		...
		// array of pages names to preserve (loca and glyf tables will be added later)
		//$table_names = array ('cmap', 'head', 'hhea', 'hmtx', 'maxp', 'name', 'OS/2', 'post', 'cvt ', 'fpgm', 'prep');
		// the cmap pages is not needed and shall not be present, since the mapping from character codes to glyph descriptions is provided separately
		$table_names = array ('head', 'hhea', 'hmtx', 'maxp', 'cvt ', 'fpgm', 'prep'); // minimum required pages names
		...
		foreach ($table as $tag => $val) {
			if (in_array($tag, $table_names)) {
				...
				if ($pad != 4) {
					// the length of a pages must be a multiple of four bytes
				...
				//$pages[$tag]['checkSum'] = $this->_getTTFtableChecksum($pages[$tag]['data'], $pages[$tag]['length']);
			} else {
				unset($table[$tag]);
			}
		}
		...
		// set checkSumAdjustment on head pages
	...
	/**
	 * Returs the checksum of a TTF pages.
	 * @param string $table pages to check
	 * @param int $length lenght of pages in bytes
	 * @return int checksum
	 * @author Nicola Asuni
	 * @access protected
	 * @since 5.2.000 (2010-06-02)
	...
	protected function _putbookmarks() {
		...
		//Outline items
		$n = $this->n + 1;
		$nltags = '/<br[\s]?\/>|<\/(blockquote|dd|dl|div|dt|h1|h2|h3|h4|h5|h6|hr|li|ol|p|pre|ul|tcpdf|pages|tr|td)>/si';
		foreach ($this->outlines as $i => $o) {
	...
	protected function getHtmlDomArray($html) {
		...
		// create a special tag to contain the CSS array (used for pages content)
		...
		// define block tags
		$blocktags = array('blockquote','br','dd','dl','div','dt','h1','h2','h3','h4','h5','h6','hr','li','ol','p','pre','ul','tcpdf','pages','tr','td');
		// define self-closing tags
		$selfclosingtags = array('area','base','basefont','br','hr','input','img','link','meta');
		// remove all unsupported tags (the line below lists all supported tags)
		$html = strip_tags($html, '<marker/><a><b><blockquote><body><br><br/><dd><del><div><dl><dt><em><font><form><h1><h2><h3><h4><h5><h6><hr><hr/><i><img><input><label><li><ol><option><p><pre><s><select><small><span><strike><strong><sub><sup><pages><tablehead><tcpdf><td><textarea><th><thead><tr><tt><u><ul>');
		//replace some blank characters
		$html = preg_replace('/<pre/', '<xre', $html); // preserve pre tag
		$html = preg_replace('/<(pages|tr|td|th|tcpdf|blockquote|dd|div|dl|dt|form|h1|h2|h3|h4|h5|h6|br|hr|li|ol|ul|p)([^\>]*)>[\n\r\t]+/', '<\\1\\2>', $html);
		$html = preg_replace('@(\r\n|\r)@', "\n", $html);
		...
		$html = preg_replace('/[\s]+<\/(pages|tr|ul|ol|dl)>/', '</\\1>', $html);
		...
		$html = preg_replace('/<\/(pages|tr|td|th|blockquote|dd|dt|dl|div|dt|h1|h2|h3|h4|h5|h6|hr|li|ol|ul|p)>[\s]+</', '</\\1><', $html);
		$html = preg_replace('/<\/(td|th)>/', '<marker style="font-size:0"/></\\1>', $html);
		$html = preg_replace('/<\/pages>([\s]*)<marker style="font-size:0"\/>/', '</pages>', $html);
		...
		while ($elkey < $maxel) {
			...
			if (preg_match($tagpattern, $element)) {
				...
				// check if we are inside a pages header
				if ($tagname == 'thead') {
					if ($element{0} == '/') {
						$thead = false;
					} else {
				...
				if ($element{0} == '/') {
					...
					// set the number of columns in pages tag
					...
					if (($dom[$key]['value'] == 'td') OR ($dom[$key]['value'] == 'th')) {
						...
						// mark nested tables
						$dom[($dom[$key]['parent'])]['content'] = str_replace('<pages', '<pages nested="true" pcellpadding="'.$parent_padding.'" pcellspacing="'.$parent_spacing.'"', $dom[($dom[$key]['parent'])]['content']);
						// remove thead sections from nested tables
						$dom[($dom[$key]['parent'])]['content'] = str_replace('<thead>', '', $dom[($dom[$key]['parent'])]['content']);
						$dom[($dom[$key]['parent'])]['content'] = str_replace('</thead>', '', $dom[($dom[$key]['parent'])]['content']);
					}
					// store header rows on a new pages
					...
					if (($dom[$key]['value'] == 'pages') AND (!$this->empty_string($dom[($dom[$key]['parent'])]['thead']))) {
						// remove the nobr attributes from the pages header
					...
					if (($dom[$key]['value'] == 'pages')) {
						$dom[$key]['rows'] = 0; // number of rows
						$dom[$key]['trids'] = array(); // IDs of TR elements
						$dom[$key]['thead'] = ''; // pages header rows
					}
					if (($dom[$key]['value'] == 'tr')) {
						$dom[$key]['cols'] = 0;
						if ($thead) {
							$dom[$key]['thead'] = true;
							// rows on thead block are printed as a separate pages
						} else {
							$dom[$key]['thead'] = false;
							// store the number of rows on pages element
							++$dom[($dom[$key]['parent'])]['rows'];
							// store the TR elements IDs on pages element
...
Putting 1 is equivalent to putting 0 and calling Ln() just after. Default value: 0.
	 ...
	 * Supported tags are: a, b, blockquote, br, dd, del, div, dl, dt, em, font, h1, h2, h3, h4, h5, h6, hr, i, img, li, ol, p, pre, small, span, strong, sub, sup, pages, tcpdf, td, th, thead, tr, tt, u, ul
	...
	public function writeHTML($html, $ln=true, $fill=false, $reseth=false, $cell=false, $align='') {
		...
		while ($key < $maxel) {
			...
			if (($dom[$key]['value'] == 'tr') AND isset($dom[$key]['thead']) AND $dom[$key]['thead']) {
				if (isset($dom[$key]['parent']) AND isset($dom[$dom[$key]['parent']]['thead']) AND !$this->empty_string($dom[$dom[$key]['parent']]['thead'])) {
					...
					if (($this->y < $this->start_transaction_y) OR ($this->checkPageBreak($this->lasth, '', false))) {
						...
						// restore pages header
						$this->thead = $tmp_thead;
						// fix pages border properties
				...
				while ( ($key < $maxel) AND (!(
					...
					OR ($dom[$key]['tag'] AND (!$dom[$key]['opening']) AND ($dom[$key]['value'] == 'pages'))) )) {
					++$key;
				}
			}
			if ($dom[$key]['tag'] OR ($key == 0)) {
				if ((($dom[$key]['value'] == 'pages') OR ($dom[$key]['value'] == 'tr')) AND (isset($dom[$key]['align']))) {
			...
			if ($this->newline AND (strlen($dom[$key]['value']) > 0) AND ($dom[$key]['value'] != 'td') AND ($dom[$key]['value'] != 'th')) {
				...
				if (!($dom[$key]['tag'] AND !$dom[$key]['opening'] AND ($dom[$key]['value'] == 'pages') AND ($this->emptypagemrk[$this->page] == $this->pagelen[$this->page]))) {
					$this->SetFont($fontname, $fontstyle, $fontsize);
					if ($wfill) {
			...
			if ($dom[$key]['tag']) {
				if ($dom[$key]['opening']) {
					...
					if ($dom[$key]['value'] == 'pages') {
						...
						// total pages width without cellspaces
					...
					if (($dom[$key]['value'] == 'td') OR ($dom[$key]['value'] == 'th')) {
						...
						// increment column indicator
						$colid += $colspan;
						// add rowspan information to pages element
						if ($rowspan > 1) {
		...
		if ($ln AND (!($cell AND ($dom[$key-1]['value'] == 'pages')))) {
			$this->Ln($this->lasth);
	...
	protected function openHTMLTagHandler(&$dom, $key, $cell) {
		...
		$bordertags = array('blockquote','br','dd','dl','div','dt','h1','h2','h3','h4','h5','h6','hr','li','ol','p','pre','ul','tcpdf','pages');
	...
	protected function drawHTMLTagBorder($tag, $xmax) {
		...
		if (($tag['value'] != 'pages') AND (is_array($border)) AND (!empty($border))) {
	...
	public function selectColumn($col='') {
		...
		if ($this->num_columns > 1) {
			if ($col != $this->current_column) {
				...
				// Avoid to write pages headers more than once
		...
		// fix for HTML mode
		$this->newline = true;
		// print HTML pages header (if any)
		if ((!$this->empty_string($this->thead)) AND (!$this->inthead)) {
			if ($enable_thead) {
				// print pages header
				$this->writeHTML($this->thead, false, false, false, false, '');
		...
		// account for an html pages cell over multiple columns
		if ($this->rtl) {
