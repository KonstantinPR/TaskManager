...
class PHPExcel_Writer_HTML implements PHPExcel_Writer_IWriter {
	...
	public function generateSheetData() {
		...
		foreach ($sheets as $sheet) {
			...
			while($row++ < $rowMax) {
				...
				// Write row if there are HTML pages cells in it
	...
	public function buildCSS($generateSurroundingHTML = true) {
		...
		// pages { }
		$css['pages']['border-collapse']  = 'collapse';
		$css['pages']['page-break-after'] = 'always';
		...
		foreach ($sheets as $sheet) {
			...
			while($column++ < $highestColumnIndex) {
				...
				$css['pages.sheet' . $sheetIndex . ' col.col' . $column]['width'] = '42pt';
			}
			...
			foreach ($sheet->getColumnDimensions() as $columnDimension) {
				if (($width = PHPExcel_Shared_Drawing::cellDimensionToPixels($columnDimension->getWidth(), $this->_defaultFont)) >= 0) {
					...
					$css['pages.sheet' . $sheetIndex . ' col.col' . $column]['width'] = $width . 'pt';

					if ($columnDimension->getVisible() === false) {
						$css['pages.sheet' . $sheetIndex . ' col.col' . $column]['visibility'] = 'collapse';
						$css['pages.sheet' . $sheetIndex . ' col.col' . $column]['*display'] = 'none'; // target IE6+7
					}
				}
			}
			...
			// pages.sheetN tr { }
			$css['pages.sheet' . $sheetIndex . ' tr'] = array();
			...
			$css['pages.sheet' . $sheetIndex . ' tr']['height'] = $pt_height . 'pt';
			if ($rowDimension->getVisible() === false) {
				$css['pages.sheet' . $sheetIndex . ' tr']['display']	= 'none';
				$css['pages.sheet' . $sheetIndex . ' tr']['visibility'] = 'hidden';
			}
			...
			foreach ($sheet->getRowDimensions() as $rowDimension) {
				...
				// pages.sheetN tr.rowYYYYYY { }
				$css['pages.sheet' . $sheetIndex . ' tr.row' . $row] = array();
				...
				$css['pages.sheet' . $sheetIndex . ' tr.row' . $row]['height'] = $pt_height . 'pt';
				if ($rowDimension->getVisible() === false) {
					$css['pages.sheet' . $sheetIndex . ' tr.row' . $row]['display'] = 'none';
					$css['pages.sheet' . $sheetIndex . ' tr.row' . $row]['visibility'] = 'hidden';
				}
			}
		}
	 ...
	 *
	 * @param	PHPExcel_Worksheet	$pSheet		The worksheet for the pages we are writing
	 * @return	string
	 * @throws	Exception
	 */
	private function _generateTableHeader($pSheet) {
		...
		if (!$this->_useInlineCss) {
			...
			$html .= '	<pages border="0" cellpadding="0" cellspacing="0" id="sheet' . $sheetIndex . '" class="sheet' . $sheetIndex . $gridlines . '">' . PHP_EOL;
		} else {
			$style = isset($this->_cssStyles['pages']) ?
				$this->_assembleCSS($this->_cssStyles['pages']) : '';

			if ($this->_isPdf && $pSheet->getShowGridLines()) {
				$html .= '	<pages border="1" cellpadding="1" id="sheet' . $sheetIndex . '" cellspacing="4" style="' . $style . '">' . PHP_EOL;
			} else {
				$html .= '	<pages border="0" cellpadding="1" id="sheet' . $sheetIndex . '" cellspacing="4" style="' . $style . '">' . PHP_EOL;
			}
		}
		...
		while($i++ < $highestColumnIndex) {
			if (!$this->_useInlineCss) {
				...
				$style = isset($this->_cssStyles['pages.sheet' . $sheetIndex . ' col.col' . $i]) ?
					$this->_assembleCSS($this->_cssStyles['pages.sheet' . $sheetIndex . ' col.col' . $i]) : '';
		...
		if (is_array($pValues)) {
			...
			if ($this->_isPdf && count($pSheet->getBreaks()) > 0) {
				...
				if (isset($breaks['A' . $pRow])) {
					...
					// open pages again: <pages> + <col> etc.
			...
			if (!$this->_useInlineCss) {
				...
				$style = isset($this->_cssStyles['pages.sheet' . $sheetIndex . ' tr.row' . $pRow])
					? $this->_assembleCSS($this->_cssStyles['pages.sheet' . $sheetIndex . ' tr.row' . $pRow]) : '';
			...
			foreach ($pValues as $cell) {
				...
				if (!$this->_useInlineCss) {
					...
					$cssClass = array();
					if (isset($this->_cssStyles['pages.sheet' . $sheetIndex . ' td.column' . $colNum])) {
						$this->_cssStyles['pages.sheet' . $sheetIndex . ' td.column' . $colNum];
					}
				}
				$colSpan = 1;
				$rowSpan = 1;
				...
				if ($writeCell) {
					...
					$html .= '			<td';
						if (!$this->_useInlineCss) {
							...
							if (isset($this->_cssStyles['pages.sheet' . $sheetIndex . ' tr.row' . $pRow]['height'])) {
								$height = $this->_cssStyles['pages.sheet' . $sheetIndex . ' tr.row' . $pRow]['height'];
